...
const DEFAULT_PERSON_COLORS=["#4F83CC","#D26A7A","#6FA05C","#B07AC4","#D18A3C","#4FA3A5","#8B6FD1","#C35F5F"];
...
function ensurePersonColors(){
 if(!state||!state.household||!state.household.people)return;
 state.household.people.forEach((p,i)=>{
   if(!p.color)p.color=DEFAULT_PERSON_COLORS[i%DEFAULT_PERSON_COLORS.length];
 });
}
...
function openCurrentProfile(email){
 ...
 currentEmail=email;state=a[email];setSession(email);ensurePersonColors();
...
function collectHousehold(){
 const people=[...document.querySelectorAll(".setup-person-name")].map((el,i)=>{
  ...
  return {id:uid(),name:el.value.trim()||`Personne ${i+1}`,role,mode,color:DEFAULT_PERSON_COLORS[i%DEFAULT_PERSON_COLORS.length],workStart:"07:00",workEnd:"17:00",days:[0,1,2,3,4],baseFreeMinutes:mode==="parttime"?180:mode==="none"?300:mode==="school"?30:60,periods:[]}
 });
...
function taskCard(t){
 const cat=state.colors.categories[t.category],pri=state.colors.priorities[t.priority],ownerObj=state.household.people.find(p=>p.id===t.ownerId),owner=ownerObj?.name||"Foyer",personColor=ownerObj?.color||"#7C8A82";
 ...
 el.innerHTML=`<div class="category-stripe" style="background:${cat}"></div><div class="task-line"><span class="task-name">${t.title}</span><span class="task-time">${fmt(t.duration)}</span></div><div class="task-meta"><span class="pill" style="background:${hexToRgba(cat,.16)};color:${cat}">${catLabel(t.category)}</span><span class="person-tag" style="background:${hexToRgba(personColor,.18)};color:${personColor};border:1px solid ${hexToRgba(personColor,.35)}"><span class="person-color-preview" style="background:${personColor}"></span>${owner}</span><span class="priority-badge">P${t.priority}</span></div><div class="task-actions"><button class="mini-btn">${t.done?"↩ Réouvrir":"✓ Terminer"}</button></div>`;
...
function renderCalendar(){
 ...
 if(state.view==="day"){const wrap=document.createElement("div");wrap.className="day-view";const side=document.createElement("div");side.className="day-sidebar";state.household.people.forEach(p=>{const a=availability(p,c);side.innerHTML+=`<div class="person-availability" style="border-left:4px solid ${p.color||"#7C8A82"}"><strong><span class="person-color-preview" style="background:${p.color||"#7C8A82"}"></span>${p.name}</strong><small>${a.label} · ${fmt(a.minutes)} disponibles</small></div>`});const main=document.createElement("div");state.instances.filter(t=>t.date===iso(c)).forEach(t=>main.appendChild(taskCard(t)));wrap.append(side,main);calendar.appendChild(wrap);return}
...
addPersonBtn.onclick=()=>{state.household.people.push({id:uid(),name:`Personne ${state.household.people.length+1}`,role:"adult",mode:"none",color:DEFAULT_PERSON_COLORS[state.household.people.length%DEFAULT_PERSON_COLORS.length],workStart:"",workEnd:"",days:[],baseFreeMinutes:300,periods:[]});saveProfile();renderPeoplePanel()};
